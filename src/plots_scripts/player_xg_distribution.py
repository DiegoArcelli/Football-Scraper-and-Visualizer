...
def get_shots_list(shot_file, team, player):
    data = pd.read_csv(shot_file)
    shots_list = []

    for shot in data.itertuples():

        if shot.team_name != team or shot.penalty or shot.player != player:
            continue
...
parser.add_argument('--player', default="Tammy Abraham", type=str)
...
matches_shots = [get_shots_list(shots_file, team, player) for shots_file in team_matches_shot_files]
...
title = f"""
{player} {team} {stat_name} simulation {league} {season} ({iters} simulations)<br>
...
title = f"""{player} {team} {stat_name} simulation {league} {season} ({iters} simulations)<br>
Probability of scoring more than {goals_scored} goals: {more}<br>
Probability of scoring less then {goals_scored} goals: {less}<br>"""
