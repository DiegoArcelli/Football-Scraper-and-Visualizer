...
data["take_ons_won_per_50_touches"] = (take_ons_won/touches)*50
take_ons_won_per_50_touches = data["take_ons_won_per_50_touches"]


x_column = "take_ons_won_per_50_touches"

text = [f"{players.iloc[i]}<br>Succ. Dribblings: {round(take_ons_won.iloc[i], 2)}<br>Succ. Dribblings per 50 touches: {round(take_ons_won_per_50_touches.iloc[i], 2)}<br>Dribbling Succ. Rate: {take_ons_succ.iloc[i]}" for i in range(len(players))]
...
scatter_plot(
    ...
    text=text,
    x_label=f'Number of successful driblings per 50 touches',
